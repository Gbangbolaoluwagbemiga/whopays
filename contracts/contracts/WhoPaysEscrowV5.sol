...
/**
 * @title WhoPaysEscrowV5
 * @dev Enterprise-grade escrow with Random Bet IDs, Claim Mapping, and Multi-sig Consensus.
 */
contract WhoPaysEscrowV5 is Ownable {
    ...
    struct Bet {
        ...
        mapping(address => string) partyClaim; // e.g., "PSG won", "Bayern won"
        mapping(address => address) partyVote; // Voting for winner
        uint256 voteCount;
        address winner;
        Status status;
        uint256 expiresAt;
    ...
    /**
     * @dev Create a bet with a unique, non-predictable ID.
     */
    function createBet(
    ...
    function getBetInfo(bytes32 betId) external view returns (
        ...
        Bet storage bet = bets[betId];
        return (bet.creator, bet.title, bet.stakePerPerson, bet.parties.length, bet.status, bet.winner, bet.isUniversal);
    }
